...
import type { BombHistoryItem } from "./bomb-types";
...
export function BombHistory({ refreshKey }: { refreshKey: number }) {
  const [items, setItems] = useState<BombHistoryItem[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const data = await api.getBombHistory({ limit: 20 });
