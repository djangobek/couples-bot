...
const awaitingBroadcast = new Map();
let botInstance = null;
/* ============================================================
   Register
   ============================================================ */
export function registerBroadcastHandler(bot) {
    botInstance = bot;
    bot.command("broadcast", async (ctx) => {
        ...
        await ctx.reply([
            ...
            reply_markup: new InlineKeyboard().text(t(lang, "broadcast.cancel"), "broadcast:cancel"),
            parse_mode: "HTML",
        });
    });
    ...
    bot.on("message:text", async (ctx, next) => {
        ...
        try {
            await ctx.api.editMessageText(progressMsg.chat.id, progressMsg.message_id, `✅ ${t(lang, "broadcast.sent", { sent })}`, { parse_mode: "HTML" });
        }
        catch {
...
/* ============================================================
   Broadcast to all
   ============================================================ */
async function sendBroadcastToAll(text) {
    if (!botInstance)
        return 0;
    ...
    for (let i = 0; i < users.length; i += BATCH) {
        ...
        await Promise.all(batch.map(async (u) => {
            try {
                await botInstance.api.sendMessage(Number(u.telegramId), text, { parse_mode: "HTML" });
                sent += 1;
            }
            catch {
                /* ignore */
