...
/* ---------- Deep-link parsers ---------- */
function inviteFromStart(text?: string): string | null {
  const m = text?.trim().match(/^\/start(?:@[^\s]+)?\s+invite_([A-Za-z0-9_-]{32,128})$/i);
  return m?.[1] ?? null;
}

function bombFromStart(text?: string): string | null {
  const m = text?.trim().match(/^\/start(?:@[^\s]+)?\s+bomb_([A-Z0-9]{6})$/i);
  return m?.[1] ?? null;
}

function tttFromStart(text?: string): string | null {
...
function miniAppUrl(params: {
  ...
  if (params.inviteCode) query.set("invite", params.inviteCode);
  if (params.bombCode) query.set("bomb", params.bombCode);
  if (params.tttCode) query.set("ttt", params.tttCode);
  const qs = query.toString();
  return qs ? `${env.WEB_APP_URL}?${qs}` : env.WEB_APP_URL;
}

/* ---------- Register ---------- */
export function registerWelcomeHandler(bot: Bot): void {
  /* ============ /start ============ */
  bot.command("start", async (ctx) => {
    ...
    try {
      ...
      if (inviteCode) {
        ...
        const kb = new InlineKeyboard().webApp(
          t(lang, "start.joinPartner"),
          url,
        );
        await ctx.reply(
          t(lang, "start.invitationReceived"),
          { reply_markup: kb },
        );
        return;
      }
