...
export function registerReferralHandler(bot: Bot): void {
  /* ============ /referral ============ */
...
async function showReferral(ctx: Context) {
  const from = ctx.from;
  if (!from) return;
  ...
  const shareText = encodeURIComponent(
    "❤️ Couples — juftlik uchun maxfiy dunyo. Do'stim, sen ham qo'shil!",
  );
