...
export function registerLanguageHandler(bot: Bot): void {
  ...
  bot.callbackQuery(/^lang:set:(uz|ru|en)$/, async (ctx) => {
    ...
    const newLang = (match?.[1] ?? "uz") as Lang;
    ...
    await db.user.update({
      where: { id: user.id },
      data: { language: newLang.toUpperCase() as "UZ" | "RU" | "EN" },
    });
...
/* ---------- Keyboard ---------- */
function languageKeyboard(current: Lang): InlineKeyboard {
  const kb = new InlineKeyboard();
  const langs: Array<{ code: Lang; label: string; emoji: string }> = [
