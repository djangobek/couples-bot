...
export class BroadcastService {
    ...
    async send(adminId, input) {
        ...
        const users = await prisma.user.findMany({
            where: input.onlyActive === false
                ? { status: { not: "DELETED" } }
                : { status: "ACTIVE" },
            select: { telegramId: true },
        });
        /* Fire and forget — do NOT await */
        void this.runBroadcast(broadcast.id, users.map((u) => u.telegramId.toString()), message, broadcast.parseMode);
        return {
            id: broadcast.id,
            sent: 0,
            failed: 0,
     ...
     */
    async runBroadcast(broadcastId, recipients, message, parseMode) {
