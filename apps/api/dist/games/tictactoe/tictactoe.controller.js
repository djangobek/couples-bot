...
export function registerTttRoutes(app) {
    /* ============================================================
       ORDER MATTERS!
    ...
    /* ---------- 2. CREATE ROOM ---------- */
    app.post("/v1/games/tictactoe/create", { preHandler: requireCouple }, async (request) => {
    ...
    /* ---------- 5. WS TOKEN ---------- */
    app.post("/v1/games/tictactoe/ws-token", { preHandler: requireCouple }, async (request) => {
        const auth = requireAuth(request);
        ...
        const { token, expiresAt } = gameTokenStore.issue(auth.id, code, "TIC_TAC_TOE");
        return success({ token, expiresAt, code });
    });
    /* ---------- 6. JOIN ROOM ---------- */
    app.post("/v1/games/tictactoe/:code/join", { preHandler: requireCouple }, async (request) => {
        const auth = requireAuth(request);
