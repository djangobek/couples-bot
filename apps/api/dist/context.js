...
import { authenticate as authenticateMiddleware, requireCouple, } from "./auth/auth.middleware.js";
/**
 ...
 */
export async function requireAuthenticatedCouple(request) {
    if (!request.auth) {
        throw new ApiError("UNAUTHORIZED", "Authentication required", 401);
    }
    await requireCouple(request);
    if (!request.couple) {
        throw new ApiError("COUPLE_NOT_FOUND", "No active couple found", 404);
    }
 ...
 */
export function getCouple(request) {
    if (!request.couple) {
        throw new ApiError("COUPLE_NOT_FOUND", "No active couple found", 404);
    }
    return request.couple;
