...
export class AdminService {
    ...
    async getDashboard() {
        ...
        const [totalUsers, activeUsers, onlineUsers, blockedUsers, newToday, newThisWeek, newThisMonth,] = await Promise.all([
            prisma.user.count(),
        ...
        const [totalGames, activeGames, finishedGames, abandonedGames, gamesToday, gamesThisWeek, gamesThisMonth, gamesByType, drawsCount,] = await Promise.all([
    ...
    /* ============================================================
       GAMES
       ============================================================ */
    async listGames(filters, pagination) {
        const where = {};
        if (filters.search) {
    ...
    async getGameDetail(gameId) {
        ...
        if (game.type === "TIC_TAC_TOE" && game.size) {
            const board = new Array(game.size * game.size).fill(null);
    ...
    `;
        ...
        const userRegistrations = dates.map((d) => ({
            date: d,
    ...
    `;
        ...
        const gamesSessions = dates.map((d) => ({
            date: d,
    ...
    `;
        ...
        const couplesCreated = dates.map((d) => ({
            date: d,
