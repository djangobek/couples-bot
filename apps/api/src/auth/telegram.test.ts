...
function makeInitData(authDate = Math.floor(NOW / 1000), mutateHash = false) {
  ...
  if (mutateHash) hash = `${hash.slice(0, -1)}${hash.endsWith("0") ? "1" : "0"}`;
...
test("rejects an invalid signature", () => {
  assert.throws(
    () => validateTelegramInitData(makeInitData(undefined, true), BOT_TOKEN, 86400, NOW),
    (error: unknown) =>
      error instanceof ApiError && error.code === "INVALID_TELEGRAM_INIT_DATA",
  );
});
...
test("rejects missing hash", () => {
  ...
  assert.throws(
    () => validateTelegramInitData(params.toString(), BOT_TOKEN, 86400, NOW),
    (error: unknown) =>
      error instanceof ApiError && error.code === "INVALID_TELEGRAM_INIT_DATA",
  );
});
...
test("rejects expired auth_date", () => {
  ...
  assert.throws(
    () => validateTelegramInitData(makeInitData(expired), BOT_TOKEN, 86400, NOW),
    (error: unknown) =>
      error instanceof ApiError && error.code === "TELEGRAM_INIT_DATA_EXPIRED",
  );
});
