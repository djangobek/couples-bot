...
import type { CreateDateInput, UpdateDateInput } from "@couples/shared";
...
export class DatesService {
  ...
  async list(userId: string, options: { upcoming?: boolean }) {
  ...
  async getById(userId: string, dateId: string) {
    const ctx = await this.ensureCouple(userId);
  ...
  async create(userId: string, input: CreateDateInput) {
    const ctx = await this.ensureCouple(userId);
  ...
  async update(userId: string, dateId: string, input: UpdateDateInput) {
    const ctx = await this.ensureCouple(userId);
  ...
  async remove(userId: string, dateId: string) {
    const ctx = await this.ensureCouple(userId);
