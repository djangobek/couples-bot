...
import type { BombRoom, BombPlayer, PublicRoomState } from "./bomb.types.js";
import {
  GAME_TIME_MS,
  GRACE_BEFORE_PAUSE_MS,
  PLACEMENT_TIME_MS,
  RECONNECT_WINDOW_MS,
  TURN_TIME_MS,
  bombCountForSize,
...
class BombRoomManager extends EventEmitter {
  private rooms = new Map<string, BombRoom>();
  ...
  async createRoom(params: {
    ...
    if (!validateSize(size)) {
      throw new ApiError(
        "VALIDATION_ERROR",
        "Grid size must be between 5 and 10",
        400,
      );
    }
    ...
    if (!trimmedReward || trimmedReward.length > 200) {
      throw new ApiError(
        "VALIDATION_ERROR",
        "Reward must be 1-200 characters",
        400,
      );
    }
    ...
    if (existingCode) {
      ...
      if (existingRoom && this.isActiveStatus(existingRoom.status)) {
        throw new ApiError(
          "GAME_IN_PROGRESS",
          "You already have an active game. Finish or leave it first.",
          409,
        );
      }
    }
  ...
  /* ============================================================
     GETTERS
     ============================================================ */
  getRoom(code: string): BombRoom | undefined {
    return this.rooms.get(code);
  }

  getRoomByUser(userId: string): BombRoom | undefined {
  ...
  /* ============================================================
     START PLACEMENT
     ============================================================ */
  private async startPlacement(room: BombRoom) {
    room.status = "PLACING";
  ...
  private async autoFillPlacement(code: string) {
    const room = this.rooms.get(code);
  ...
  private async startPlaying(room: BombRoom) {
    room.status = "PLAYING";
  ...
  /* ============================================================
     READY
     ============================================================ */
  async playerReady(code: string, userId: string, bombs?: number[]) {
    const room = this.rooms.get(code);
    ...
    if (bombs && Array.isArray(bombs) && bombs.length > 0) {
      ...
      if (!validateBombs(bombSet, room.size, room.bombCount)) {
        throw new ApiError(
          "VALIDATION_ERROR",
          `You must place exactly ${room.bombCount} bombs`,
          400,
        );
      }
  ...
  async clickCell(
    ...
    row: number,
    col: number,
  ): Promise<{ hit: boolean; gameOver: boolean }> {
    const room = this.rooms.get(code);
    if (!room) throw new ApiError("GAME_NOT_FOUND", "Game not found", 404);
    ...
    if (!opp) throw new ApiError("INVALID_STATE", "Opponent missing", 500);
  ...
  /* ============================================================
     FINISH
     ============================================================ */
  private async finishGame(room: BombRoom, winnerId: string) {
  ...
  private async expireGame(code: string) {
    const room = this.rooms.get(code);
    if (!room || room.status !== "PLAYING") return;
    ...
    if (p0.hits > p1.hits) winnerId = p0.userId;
    else if (p1.hits > p0.hits) winnerId = p1.userId;
  ...
  private async maybeAutoPause(code: string) {
    const room = this.rooms.get(code);
    if (!room) return;
  ...
  /* ============================================================
     ...
     ============================================================ */
  private async pauseGame(room: BombRoom, pausedByUserId: string) {
    if (room.status === "PAUSED") return;
    if (this.isFinishedStatus(room.status)) return;
  ...
  /* ============================================================
     RESUME
     ============================================================ */
  private async resumeGame(room: BombRoom) {
    if (room.status !== "PAUSED") return;
  ...
  /* ============================================================
     ...
     ============================================================ */
  private scheduleReconnectTimeout(code: string) {
    this.clearReconnectTimer(code);
  ...
  private async handleReconnectTimeout(code: string) {
    const room = this.rooms.get(code);
  ...
  /* ============================================================
     ABANDON
     ============================================================ */
  private async abandonRoom(room: BombRoom) {
  ...
  /* ============================================================
     PUBLIC STATE
     ============================================================ */
  getPublicState(code: string, userId: string): PublicRoomState {
    const room = this.rooms.get(code);
    if (!room) throw new ApiError("GAME_NOT_FOUND", "Game not found", 404);
    ...
    return {
      ...
      players: room.players
        .filter((p): p is BombPlayer => p !== null)
        .map((p) => ({
          userId: p.userId,
          seat: p.seat,
