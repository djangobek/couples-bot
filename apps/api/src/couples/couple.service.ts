...
export class CoupleService {
  ...
  async getMembers(userId: string) {
    ...
    if (!current) throw new ApiError("COUPLE_MEMBERSHIP_REQUIRED", "An active couple membership is required", 403);
  ...
  async createInvite(userId: string) {
    ...
    if (count >= 2) throw new ApiError("COUPLE_FULL", "This couple already has two members", 409);
  ...
  async resolveInvite(code: string) {
    ...
    if (!invite) throw new ApiError("INVITE_NOT_FOUND", "Invite not found", 404);
    if (invite.status === "ACCEPTED") throw new ApiError("INVITE_ALREADY_ACCEPTED", "Invite has already been accepted", 409);
    if (invite.status === "REVOKED") throw new ApiError("INVITE_REVOKED", "Invite has been revoked", 409);
    ...
    if (invite.couple.status !== "ACTIVE") throw new ApiError("INVITE_NOT_FOUND", "Couple is no longer active", 404);
  ...
  async acceptInvite(userId: string, code: string) {
    return prisma.$transaction(async (tx) => {
      ...
      if (invite.couple.status !== "ACTIVE") throw new ApiError("INVITE_NOT_FOUND", "Couple is no longer active", 404);
      if (invite.senderId === userId) throw new ApiError("SELF_INVITE", "You cannot accept your own invite", 409);

      const ownMembership = await tx.coupleMember.findFirst({
        where: { userId, leftAt: null, couple: { status: "ACTIVE" } },
        select: { id: true },
      });
      if (ownMembership) throw new ApiError("ALREADY_IN_COUPLE", "You are already in an active couple", 409);

      const members = await tx.coupleMember.count({ where: { coupleId: invite.coupleId, leftAt: null } });
      if (members >= 2) throw new ApiError("COUPLE_FULL", "This couple already has two members", 409);
  ...
  async leaveCouple(userId: string) {
    return prisma.$transaction(async (tx) => {
      ...
      if (!membership) throw new ApiError("COUPLE_MEMBERSHIP_REQUIRED", "An active couple membership is required", 403);
